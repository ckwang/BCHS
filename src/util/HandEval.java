...
public class HandEval {
										...
										do {
											...
											SUIT_COUNT = (suits[card_1] == suits[FLUSH_SUIT_INDEX] ? 1
													...
													+ (suits[card_6] == suits[FLUSH_SUIT_INDEX] ? 1
															: 0)
													+ (suits[card_7] == suits[FLUSH_SUIT_INDEX] ? 1
															: 0);
											CARDS_MATCHED_SO_FAR += SUIT_COUNT;
										} while (CARDS_MATCHED_SO_FAR < 3
												&& FLUSH_SUIT_INDEX < 4);
	...
	public double[] computeFlopEquityForSpecificCards(int[] holeCards,
		...
		do {
			...
			long table_key_cumulative_to_4 = flop_table_key
					+ deckcardsKey[table_card_4];
			int j = 0;
			do {
...
//				equity_percentage[6], equity_percentage[7]);
	...
	public double[] computeTurnEquityForSpecificCards(int[] holeCards,
		...
		int table_card_2 = tableCards[1];
		int table_card_3 = tableCards[2];
		int table_card_4 = tableCards[3];

		long turn_table_key = deckcardsKey[table_card_1]
				+ deckcardsKey[table_card_2] + deckcardsKey[table_card_3]
				+ deckcardsKey[table_card_4];
		...
		int i = 0;
		do {
			int table_card_5 = undealtCards[i];
			long table_key = turn_table_key + deckcardsKey[table_card_5];
			// Reset table_flush_key.
			int table_flush_key = UNVERIFIED;
			// Reset number of interested players.
			number_of_players_sharing_the_pot = 1;
			...
			do {
				...
				if (current_best_rank < next_rank) {
					...
					// Reset number of players interested
					number_of_players_sharing_the_pot = 1;
				} else if (current_best_rank == next_rank) { // Case: pot shared
																// with another
																// player
				// Increment by 1 the number of interest players
					number_of_players_sharing_the_pot++;
...
//				equity_percentage[6], equity_percentage[7]);
	...
	public double[] computeRiverEquityForSpecificCards(int[] holeCards,
		...
		int card_1 = holeCards[0];
		int card_2 = holeCards[1];

		int current_best_rank = getRankOfSeven(card_1, card_2, tableCards[0],
		...
		for (int i = 1; i < number_of_players; i++) {
			card_1 = holeCards[2 * i];
			card_2 = holeCards[2 * i + 1];
			next_rank = getRankOfSeven(card_1, card_2, tableCards[0],
					tableCards[1], tableCards[2], tableCards[3], tableCards[4]);
		...
		long pot_share = CAKE / number_of_players_sharing_the_pot;
...
//				equity_percentage[6], equity_percentage[7]);
	...
	void timeRankMethod() {
		...
		// Time how long it takes to enumerate the hands. I'm not sure if this
		// really affects the result much.
		...
		// The incremental additions (N++ and N+=7) here aren't really the work
		// of the evaluator and should be subtracted from the total time to get
		// a fairer
		// estimate of the work, although this doesn't materially affect the
		// outcome.
		for (int N = 0; N < BIG_NUMBER; N += 7) {
			...
			if (FLUSH_SUIT < 0) {
				KEY = (KEY >> NON_FLUSH_BIT_SHIFT);
				rank = rankArray[KEY < CIRCUMFERENCE_SEVEN ? (int) KEY
						: (int) KEY - CIRCUMFERENCE_SEVEN];
			} else {
				int FLUSH_KEY = (deckcardsSuit[card_1] == FLUSH_SUIT ? deckcardsFlush[card_1]
						: 0)
		...
		System.out
				.println("\ntime taken: "
						+ (finish - start - (finish_adjust - start_adjust))
						+ " millis");

		float rate = BIG_NUMBER / 7 * (1000 + 0.0f)
				/ (finish - start - (finish_adjust - start_adjust) + 0.0f);
